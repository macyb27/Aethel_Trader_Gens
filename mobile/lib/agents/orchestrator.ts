 ...
 * 
 * Workflow: Hypothesis → Backtest → Paper Trade → Validation → Live
 * 
 * Agents:
...
export class AgentOrchestrator {
  ...
  private async runLivePhase(): Promise<void> {
    ...
    this.sendMessage({
      ...
      priority: "critical",
      payload: {
        message: "🚀 GOING LIVE - Rule activated for real trading",
  ...
  emergencyHalt(reason: string): void {
    ...
    this.sendMessage({
      ...
      priority: "critical",
      payload: {
        message: "🛑 EMERGENCY HALT - All trading stopped",
        reason,
      },
    });
  }
  ...
  private sendMessage(message: AgentMessage): void {
    ...
    console.log(`[${message.priority.toUpperCase()}] ${message.from} → ${message.to}:`, message.payload);
  }
